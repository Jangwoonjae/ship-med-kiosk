...
export default function CategorySelect({ counts, onSelect }: CategorySelectProps) {
  return (
    <div className="flex flex-col items-center gap-6 w-full max-w-3xl mx-auto px-4">
      <h2 className="text-3xl font-bold text-gray-800">분류를 선택하세요</h2>
      <div className="grid grid-cols-3 gap-4 w-full">
        {CATEGORIES.map(cat => (
          ...
          >
            ...
            <span className="text-2xl font-bold text-gray-800">{cat.label}</span>
            <span className="text-base text-gray-500">{cat.desc}</span>
            <span className="text-sm font-medium text-blue-600">{counts[cat.key] ?? 0}품목</span>
          </button>
        ))}
      </div>
    </div>
