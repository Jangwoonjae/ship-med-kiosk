    ...
    <div className="min-h-screen bg-gray-50 flex flex-col">
      <header className="bg-gray-900 text-white px-6 py-4 flex items-center justify-between">
        ...
        <div className="flex gap-3">
          <a href="/" className="text-gray-400 text-sm hover:text-white">← 선원 화면</a>
          <button onClick={() => setAuthed(false)} className="text-gray-400 text-sm hover:text-red-400">로그아웃</button>
        </div>
      </header>

      <nav className="bg-white border-b px-6 flex gap-0">
        {tabs.map(t => (
          <button
            key={t.key}
            onClick={() => setTab(t.key)}
            className={`
              px-5 py-4 text-base font-medium border-b-2 -mb-px transition-colors
